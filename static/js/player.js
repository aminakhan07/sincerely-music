...
const record = el('#record'), art = el('#art'), title = el('#title'), artist = el('#artist'), play = el('#play'), seek = el('#seek'), time = el('#time'), status = el('#status'), login = el('#login');
...
async function update() {
  try {
    ...
    record.classList.toggle('spinning', data.playing); play.textContent = data.playing ? 'Ⅱ' : '▶'; play.disabled = false; seek.disabled = false;
